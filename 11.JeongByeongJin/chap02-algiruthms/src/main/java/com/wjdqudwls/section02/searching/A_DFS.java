package com.wjdqudwls.section02.searching;
...
public class A_DFS {
  ...
  public static Integer solution(String input) throws IOException {
    ...
    map = new int[node + 1][node + 1]; // index 0 사용 X
    ...
    for (int i = 0; i < edge; i++) {
      st = new StringTokenizer(br.readLine()); // 1 2로 입력 (공백 기준으로 토큰 나눔)
      // "1 3" -> "1", "3" / 2개의 토큰으로 자동으로 나뉨
      int a = Integer.parseInt(st.nextToken());
      int b = Integer.parseInt(st.nextToken());

      // 무방향 그래프 == 양쪽 모두 1로 표기
      map[a][b] = map[b][a] = 1; // 대입연산자는 오른쪽부터 시작, 1줄로 적을 수 있음

    }

    /* DFS 탐색 시작 */

    System.out.println("1번 컴퓨터 감염 시작");
    // 1. 재귀 호출 방법
//    dfsRecursive(1); // 1번부터 탐색을 시작
  ...
  private static void dfsStack(int start){
    ...
    => Queue를 쓸 때는 BFS일때 쓴다. DFS/Stack */
    ...
    while(!stack.isEmpty()){
      ...
      System.out.println(current + "번 노드는 현재 노드입니다.");
      ...
      if(!sb.isEmpty())
        System.out.println("현재 노드에서"+sb.toString()+"번째 노드를 스택에 추가했습니다.");
      else System.out.println("현재 노드에서 스택에 추가한 노드는 없습니다.");
