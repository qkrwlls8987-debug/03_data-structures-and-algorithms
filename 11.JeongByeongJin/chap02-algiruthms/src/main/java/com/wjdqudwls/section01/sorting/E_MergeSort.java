...
public class E_MergeSort {
  ...
  /* 문제 : n개의 정수가 주어졌을 때, 병합 정렬 알고리즘을 사용하여 오름차순 정렬하는 프로그램 작성하기 */
  public static void solution(int[] arr) {

    System.out.println("원본 배열 : " + Arrays.toString(arr));

    // 병합 정렬에 필요한 임시 배열 생성(추가메모리 공간)
  ...
  private static void merge(int[] arr, int[] temp, int left, int mid, int right) {
    ...
    while (leftIndex <= mid && rightIndex <= right) {
      ...
      // 임시 배열의 leftIndex가 가리키는 값이 rightIndex가 가리키는 값보다 같거나 작은 경우
      if (temp[leftIndex] <= temp[rightIndex]) {
        arr[current++] = temp[leftIndex++];
      } else {
        // 임시 배열의 rightIndex가 가리키는 값이 leftIndex가 가리키는 값보다 같거나 작은 경우
