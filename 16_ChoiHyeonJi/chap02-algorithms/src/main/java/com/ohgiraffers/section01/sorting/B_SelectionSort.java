...
* - 배열을 여러 번 반복하면서, 선택한 요소와 나머지 요소들을 비교하여 정렬
* - 시간 복잡도 : O(n^2)
* - 하지만 버블 정렬보다 값 교환 횟수가 적은 편이다.   //작은거만 찾아서 바꿈
...
public class B_SelectionSort {
  /* 문제 : n개의 정수가 주어졌을 때 선택 정렬 알고리즘을 사용하여
오름차순 정렬하는 프로그램 작성하기 */
    ...
    for(int i=0; i< arr.length - 1; i++){
      ...
      for(int j = i + 1; j < arr.length; j++){
          if(arr[minIndex] > arr[j]){
            //swap(arr, minIndex, j); // 원본 배열 자체에 스왑이 일어남
            minIndex = j;
          }
      }

      swap(arr, minIndex, i); // 좀 빨라짐
