...
* - 모든 노드의 왼쪽 서브트리에 있는 모든 노드의 값은 현재 노드값 보다 작다.
* - 모든 노드의 오른쪽 서브트리에 있는 모든 노드의 값은 현재 노드값 보다 크다.
* - 모든 서브트리 또한 이진 탐색 트리다.
* - 이러한 정렬 속성 덕분에 탐색, 삽입, 삭제 연산을 효율적으로 수행 가능함.
* (평균 O(log n)) 시간을 가짐. 평균 시간 복잡도
* - 하지만 한 쪽으로 편향된 트리는 최악의 경우 O(n)이 될 수 있다.
*  부모보다 작은 애 왼/ 큰 애 오른 배치
* */
...
public class BinarySearchTree<T extends Comparable<T>> {
  ...
  /**
   * 노드 삽입을 위한 재귀 헬퍼 메서드 => 헬퍼 메서드 ? 다른 메서드 도와주는 메서드
   *
  ...
  private Node<T> insertRec(Node<T> node, T data) {
    ...
    if (data.compareTo(node.data) < 0) { // compareTo 값 비교 // 양수,음수
      node.left = insertRec(node.left, data);  // 재귀호출
    }
  ...
  private void preOrderRec(Node<T> node, List<T> result){
    if(node != null) {
      result.add(node.data); // 현재 노드 값을 기록  -> Root Node 방문
  ...
  public boolean search(T data) {
    return searchRec(root, data); // root 부터 검색
  }
  ...
  private boolean searchRec(Node<T> node, T data) {
    ...
    return data.compareTo(node.data) < 0 // 노드보다 데이터 작으면 왼쪽
  ...
  private Node<T> deleteRec(Node<T> node, T data) {
    if (node == null) return node; // 삭제할 데이터가 없음
    /* 삭제할 노드 검색 */
    if (data.compareTo(node.data) < 0) {
      ...
      node.data = minValue(node.right); // node.data =오른쪽 서브트리 가장 작은 값;

      // 제일 작은 값을 가지는 노드를 삭제
      node.right = deleteRec(node.right, node.data);// (오른쪽에서 찾음, 가장 작은값 node.data)
    }
    return node;
  }
  ...
  public static void main(String[] args) {
    BinarySearchTree<Integer> bst = new BinarySearchTree(); //타입 Integer로 제한
    // 트리 만들기
    bst.insert(50);
    bst.insert(30);
    bst.insert(70);
    ...
    System.out.println("전위 순회 : " + bst.preOrder()); //list toString() 호출하면 안이 다 보임
