...
*   - 분할 후 각각의 부분 배열을 재귀적으로 정렬하여 최종적으로 모두 정렬된 형태를 완성한다
*
* 시간 복잡도
*   - O(nlogn): 평균
*   - O(n^2): 최악
*
*   - 메모리 사용이 적고, 대부분 데이터 정렬에서 좋은 성능을 보여줌
* */
...
public class D_QuickSort {
    ...
    /**
     * 전달 받은 arr에 pivot을 지정하고, low, high를 이동시키면서 알맞게 값을 교환하는 작업
     * @param arr
     * @param low
     * @param high
    ...
    private static int partition(int[] arr, int low, int high) {
        ...
        // low와 high가 교차될 때 까지 바복
        while(low <= high) {
            // 왼쪽부터 pivot까지 크거나 같은 값을 찾을 때 까지 low증가
            while (arr[low] < pivot) { low++; }

            // 오른쪽부터 pivot까지 작거나 같은 값을 찾을 때 까지 high감소
            while (arr[high] > pivot) { high--; }

            // low, high의 이동이 끝났을 때 교차되지 않는 경우
            if (low <= high) {
                swap(arr, low, high);
                low++;
                high--;
