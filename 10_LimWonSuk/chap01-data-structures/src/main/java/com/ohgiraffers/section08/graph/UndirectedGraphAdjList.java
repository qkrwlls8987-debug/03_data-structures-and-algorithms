...
public class UndirectedGraphAdjList {
    // 그래프의 총 정점(노드) 개수
    private int vertices; // 전체 vertices 저장
    // 각 정점의 인접 노드 목록을 저장하는 리스트의 리스트
    // adjList.get(i)는 i번 정점과 연결된 노드들의 리스트
    private List<List<Integer>> adjList; // 인접 리스트 제작
    // 각 노드의 활성 상태. true이면 해당 노드가 존재 (동적 노드 추가/삭제 지원)
    private List<Boolean> active; // 활성화된 목록 제작

    /**
     * 지정된 수의 정점으로 그래프 초기화
     * @param vertices 초기 정점의 수
     */
    public UndirectedGraphAdjList(int vertices) { // 생성자
        this.vertices = vertices;
        adjList = new ArrayList<>(); // adj 리스트
        active = new ArrayList<>(); // 활성화 목록
     ...
     * @param v 연결할 정점 2
     */
    public void addEdge(int u, int v) { // u=0, v=1 / u=0, v=2 / u=0, v=3 순으로 진행
    ...
    public void removeNode(int x) {
        if (x < vertices && active.get(x)) {
            ...
            adjList.get(x).clear(); // 안에있던 내용 제거
        }
    }
