...
* 무방향 빅다중치 그래프 (UndirectedGraphAdjMatrix) 구현
*
* - 2차원 배열(행렬)을 사용하여 모든 정점 쌍의 연결 여부를 표현 합니다.
* - matrix[i][j] = true -> 정점 i와  j가 연결되어 있음을 의미합니다.
* - 밀집그래프(Dense Graph, 간선이 많은 그래프)에서 효율적입니디.
* */
public class UndirectedGraphAdjMatrix {
    ...
    private List<Boolean> active; // true false 형태로 하고 false시 연결구조를 해제
    ...
    public UndirectedGraphAdjMatrix(int vertices) {
        ...
        active = new ArrayList<>(); // 현재 활성화 상태
    ...
    public static void main(String[] args) {
         ...
         * 2 -- 3
         */
        graphMatrix.addEdge(0, 1); // 0과1 1과0이 연결된다
    ...
    public int addNode() {
        ...
        for (int i = 0; i < newVertexIndex; i++) {
            for (int j = 0; j < newVertexIndex; j++) {
                newMatrix[i][j] = matrix[i][j]; // 5 * 5 값에 이전 배열 입력
            }
        }
        matrix = newMatrix; // 내부 행렬의 참조를 새 행렬로 교체
        active.add(true);   // 새 노드를 활성 상태로 설정
        return newVertexIndex; // 인덱스 반환
    }
    ...
    public void printGraph() {
        ...
        System.out.print("  "); // 출력시 첫줄 띄어쓰기
        for (int i = 0; i < vertices; i++) {
            if (active.get(i)) {
                System.out.print(i + " ");
            } else {
                System.out.print("x "); // 아니면 X
            }
        }
        System.out.println();
        ...
        for (int i = 0; i < vertices; i++) {
            ...
            for (int j = 0; j < vertices; j++) {
                ...
                if (!active.get(i) || !active.get(j)) {
                    System.out.print("x "); // 비활성화면 X
                } else {
                    System.out.print((matrix[i][j] ? 1 : 0) + " "); // 아니면 출력
                }
            }
            System.out.println();
        }
