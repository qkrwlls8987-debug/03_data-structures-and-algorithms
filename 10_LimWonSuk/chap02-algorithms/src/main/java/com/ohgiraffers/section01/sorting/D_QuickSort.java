...
public class D_QuickSort {
      /*  문제 : n개의 정수가 주어졌을 때 퀵 정렬 알고이즘을 사용하여
     오름 차순 정렬하는 프로그램 작성하기 */
    ...
    /**
     * 전달 받은 arr에 피벗을 지정하고,
     * low, high를 이동 시키면서 알맞게 값을 교환하는 작업 수핼
     * @param arr
     * @param low
     * @param high
    ...
    private static int partition(int[] arr, int low, int high) {
        ...
        // low와 high가 교차 될 때 까지 반복
        while(low <= high){
            while (arr[low] < pivot) low++; // 왼쪽 부터 pivot 까지 크거나 같은 값을 찾을 때 까지 low 증가
            while (arr[high] > pivot) high--; // 오른쪽 부터 pivot 까지 작거나 같은 값을 찾을 때 까지 high 감소
