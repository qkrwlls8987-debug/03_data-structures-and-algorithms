package com.ohgiraffers.section01.sorting;

import java.util.Arrays;

/* 삽입 정렬
 * - 이미 정렬된 부분에 새로운 요소를 적절한 위치에 삽입하는 정렬
 * - 왼쪽 부터 차례대로 확장하며,
 *    새로운 요소가 들어갈 위치를 찾기 위해 비교하면서 이동함.
 * - 시간 복잡도
 * -- 평균 : O(n^2)
 * -- 최저 : O(n) -> 이미 정렬된 상태일 경우다 뿅 ♥
 * */
public class C_InsertionSort {
    /*  문제 : n개의 정수가 주어졌을 때 삽입 정렬 알고이즘을 사용하여
     오름 차순 정렬하는 프로그램 작성하기 */
    public static void solution(int[] arr) {
        // 최소값 찾기
        System.out.println("초기 값 : " + Arrays.toString(arr));

        for (int i = 1; i < arr.length; i++) { // 0번은 기준점이다.
            int temp = arr[i]; // 삽입될 값을 임시 변수에 저장

            int j; // j를 for문 밖으로 뺌 (for 문이 끝나보 j 를 써야하기 때문에)

            for ( j = i - 1; j >= 0; j--) { // temp 값은 i에 해당하는 배열의 숫자값
                if (arr[j] > temp) { // if arr의 j번째가 임시저장한것보다 크다면 True
                    arr[j + 1] = arr[j];// 위의값을 여기에 대입하겠다
                } else {
                    break;
                }
            }
            arr[j + 1] = temp; // for문종료후 j값 입력 (arr[j + 1] = temp; arr값에 temp값 대입) temp값은 배열의 해당하는 숫자

            System.out.println((i + 1) + "번째 : " + Arrays.toString(arr));

        }
    }

}
