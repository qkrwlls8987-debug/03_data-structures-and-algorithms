...
public class MyLinkedList<T> {
  ...
  /* 정적 내부 클래스
  * static -> 공유(STatic)영역에 한개만, 한번만 올라감
  ...
  * */
  ...
  public MyLinkedList(){
    ...
    tail = null;
    size = 0;
    // JVm내에서 자동 초기화 되긴 하지만 그래도 명시적으로 써주면 좋음
  }

  /**
   * 리스트의 끝에 전달받은 요소를 추가
   * 시간 복잡도 : O(1)
   * -> tail 포인터를 이용해서 바로 추가 가능
   * @param element
   */
  ...
  public void add(int index, T element){
    ...
    if(index == 0){
      ...
      tail = newNode; // head포인터가 새 노드를 가리키도록 지정
      if(size == 0){
        // 기존에 저장된 요소가 없다면 head, tail 모두 newNode를 가르켜야함
        tail = newNode;
      }
    }
