...
class C_InsertionSortTest {
  ...
  static void setUp(){
    ...
    act2 = new int[]{72, 25, 23, 99, 10, 24, 12, 25};
    exp2 = new int[]{10, 12, 23, 24, 25, 25, 72, 99};
  ...
  void testInsertionSort(int[] act, int[] exp){
    C_InsertionSort.solution(act); // 원본(act자체)가 정렬됨
    Assertions.assertArrayEquals(exp, act); //배열 요소가  모두 같으면 성공
  }
}