...
public class D_QuickSort {
  ...
  /**
   * 전달 받은 arr의 pivot을 지정하고,
   * low, high를 이동 시키면서 알맞게 값을 서로 교환하는 작업
   * @param low
   * @param high
   * @return 최종 pivot의 인덱스 -> pivot을 기준으로 부분 배열을 만들 때, 사용
   */
  private static int partition(int[] arr, int low, int high){
    ...
    while(low <= high) {
      ...
      // 왼쪽부터 pivot 까지 크거나 같은 값을 찾을 때 까지, low 증가
      while(arr[low] < pivot) low++;
      // 오른쪽부터 pivot 까지 작거나 같은 값을 찾을 때 까지, high 감소
